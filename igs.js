...
// Given the start and end coordinates of a line, and a desired Y-coordinate, 
// this function returns the X coordinate that corresponds to the Y.
// Adapted from https://stackoverflow.com/a/12368954/566307
function get_bresenham_x(start_x, start_y, end_x, end_y, wanted_y) {
	// // Algorithm:
	...
	// Only proceed if the desired Y coordinate is within the bounds of the line's start/end Y coordinates
	if ( (wanted_y >= Math.min(start_y, end_y)) && (wanted_y <= Math.max(start_y, end_y)) ) {
		// I have rearranged the equation above to solve for X rather than Y
		const wanted_x = (wanted_y * (end_x - start_x)) / (end_y - start_y) - start_y + start_x;
		return wanted_x;
	}
	return null;
}
...
function GetYOfLineAtX(x0, y0, x1, y1, x) {
	// Only proceed if the desired X coordinate is within the bounds of the line's start/end X coordinates
...
function GetXOfLineAtY(x0, y0, x1, y1, y) {
	// Only proceed if the desired Y coordinate is within the bounds of the line's start/end Y coordinates
...
const flood = {
	visited: [],
	begin: function(ctx, x, y, new_color, border_color, polygon=null) {

		let match_color = -1;
		if (border_color == -1) {
			match_color = pixel_color_to_word32(ctx, x, y);
			debug(`match_color: ${match_color}`);
		}

		debug(`border_color: ${border_color}`);

		// Begin the fill
		this.fill(ctx, x, y, new_color, border_color, match_color, polygon);
	},

	// This flood fill can serve as a normal Seed Fill, or as a Boundary Fill.
	// For the purpose of creating filled polygons, the Boundary Fill is preferred, 
	// to ensure various colors underneath are properly converted.

	fill: function(ctx, x, y, new_color, border_color, match_color, polygon=null) {
		// // If this pixel is outside the polygon, then don't bother trying to fill it.
		// if (polygon != null) {
		// 	if (!isInPoly([x,y], polygon)) {
		// 		return;
		// 	}
		// }

		// // If we're looking for a match color, and the pixel is match color, then fill.
		// // If we're looking for anything but a border color, and the pixel doesn't match, then fill. 
		// if ((border_color == -1 && pixel_rgb == match_color) || (match_color == -1 && pixel_rgb != border_color)) {
		// }
		...
		const p32 = new Uint32Array(img_data.data.buffer);
		const stack = [x + y * w]; // add starting pos to stack
		// debug(p32);
		// debug(stack);

		if (match_color === new_color || match_color === undefined) { return } // avoid endless loop

		while (stack.length) {
			let idx = stack.pop();
			while (
				idx >= w && (
					(border_color == -1 && p32[idx - w] === match_color) || 
					(match_color == -1 && p32[idx - w] !== border_color)
				)
			) { idx -= w }; // move to top edge
			right = left = false;   
			left_edge = (idx % w) === 0;          
			right_edge = ((idx +1) % w) === 0;
			while (
				p32[idx] !== undefined && (
					(border_color == -1 && p32[idx] === match_color) || 
					(match_color == -1 && p32[idx] !== border_color)
				)
			) {
				p32[idx] = new_color;
				if(!left_edge) {
					if (
						(border_color == -1 && p32[idx - 1] === match_color) || 
						(match_color == -1 && p32[idx - 1] !== border_color)
					) { // check left
						if (!left) {        
...
// // Adapted from code by Darel Rex Finley
// // https://alienryderflex.com/polygon_fill/
// // ----------------------------------------
// // Josh: I added Math.round() to cast the calculated X coordinates to (int) since this is an oldschool pixel grid.
// // Also refactored to let the function itself build the number of points, and the x- and y-coord arrays.
...
// 	//  Loop through the rows of the image.
// 	for (let pixel_y=canvas_top; pixel_y<canvas_bottom; pixel_y++) {
// 		let num_nodes, x_nodes=[], pixel_x, i, j, swap;
// 		// console.log(`pixel_y: ${pixel_y}`);

// 		//  Build a list of num_nodes.
// 		num_nodes=0; j=num_points-1;
// 		for (i=0; i<num_points; i++) {

// 			// ORIGINAL CODE
// 			// if (y_coords[i] < pixel_y && y_coords[j] >= pixel_y || y_coords[j] < pixel_y && y_coords[i] >= pixel_y) {
// 			// 	let poss_node = x_coords[i] + (pixel_y - y_coords[i]) / (y_coords[j] - y_coords[i]) * (x_coords[j] - x_coords[i]);
// 			// 	if (!isNaN(poss_node)) {
// 			// 		x_nodes[num_nodes++] = Math.round(poss_node);				
// 			// 	}
// 			// }
...
// 			// console.log(`	start: ${x_coords[i]}, ${y_coords[i]}\tend: ${x_coords[j]}, ${y_coords[j]}\t`);
// 			const poss_node = GetXOfLineAtY(x_coords[i], y_coords[i], x_coords[j], y_coords[j], pixel_y);


// 			const poss_node_b = get_bresenham_x(x_coords[i], y_coords[i], x_coords[j], y_coords[j], pixel_y);

// 			// console.log(`	poss_node  : ${Math.round(poss_node)}`);
// 			// console.log(`	poss_node_b: ${Math.round(poss_node_b)}`);
...
// 		//  Sort the num_nodes, via a simple “Bubble” sort.
// 		i=0;
// 		while (i<num_nodes-1) {
// 			if (x_nodes[i]>x_nodes[i+1]) {
// 				swap=x_nodes[i]; x_nodes[i]=x_nodes[i+1]; x_nodes[i+1]=swap; 
// 				if (i) { i--; }
// 			}
// 			else {
// 				i++;
...
// 			if (x_nodes[i] >= canvas_right) {
// 				break;
// 			}
// 			if (x_nodes[i+1] > canvas_left ) {
// 				if (x_nodes[i] < canvas_left ) {
// 					x_nodes[i] = canvas_left;
// 				}
// 				if (x_nodes[i+1] > canvas_right) {
// 					x_nodes[i+1] = canvas_right;
// 				}
// 				// alternate for correcting missing col: (pixel_x=x_nodes[i]; pixel_x<=x_nodes[i+1]; pixel_x++)
// 				for (pixel_x=x_nodes[i]; pixel_x<x_nodes[i+1]; pixel_x++) { 
// 					fill_pixel(ctx, pixel_x, pixel_y);
// 				}
// 			}
// 		}
// 	}
